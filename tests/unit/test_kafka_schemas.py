...
def test_event_envelope_wraps_payload_and_stamps_topic_as_event_type():
    ...
    envelope = EventEnvelope.wrap(Topic.ORDER_CREATED, payload)

    assert envelope.event_type == Topic.ORDER_CREATED.value
...
def test_envelope_round_trips_through_json_like_a_real_kafka_message():
    ...
    assert decoded["event_type"] == Topic.ORDER_CREATED.value
